...
class EditTeaching {
  ...
  //Handle API put method and submit button animation
  async handleSubmit(e) {
    ...
    if (this.form.elements.text.value.split("").length > 0) {
      ...
      const editTeaching = await TeachingsApi.editTeaching(this.id, teaching);
