...
class TeachingList {
  ...
  //Teaching filter button highlighting
  filterBtn() {
  ...
  addEventListeners() {
    this._teachingListEl.addEventListener("click", (e) => {
      ...
      if (e.target.classList.contains("delete")) {
        ...
        const editCard = document.querySelector(`[data-id="${teachingId}"]`);

        const editTextAreaEl = document.querySelector(".edit-textarea");
        editTextAreaEl.value = editCard.children[3].textContent;

        const editTagEl = document.querySelector(".edit-tag");
        editTagEl.value = editCard.children[4].textContent;
      }
    });
  }

  //Get teachings from API
  async getTeachings() {
